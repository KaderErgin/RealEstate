...
namespace RealEstate_Dapper_Api.Repositories.EmployeeRepositories
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public async Task<List<ResultEmployeeDto>> GetAllEmployee()
        {
            ...
            using (var connection = _context.CreateConnection())
            {
                var value = await connection.QueryAsync<ResultEmployeeDto>(query);
                return value.ToList();
            }
        }
        ...
        public async Task<GetByIDEmployeeDto> GetEmployee(int id)
        {
            ...
            using (var connection = _context.CreateConnection())
            {
                var values = await connection.QueryFirstAsync<GetByIDEmployeeDto>(query, parameters);//tek deger dondurecek
                return values;
            }
        }
